...
class ItemScrollController {
  ...
  /// 
  /// Sometimes, when item position is not available, because it's too far,
  /// the scroll won't take up a complete path to this item, instead it will show
  /// an animation that's composed of three parts:
  /// 
  ///  1. The list view starts scrolling
  ///  2. Another list view, which scrolls with the same speed, fades over the first one
  /// and shows items that are close to the scroll target
  ///  3. The second list view scrolls and stops on the target
  /// 
  /// The [opacityAnimationWeights] can be used to apply custom weights theses three stages
  /// of this animation. The default weights `[40, 20, 40]` are good with default [Curves.linear].
  /// In other cases, for example, if you use [Curves.easeOut] consider something like `[20, 20, 60]`.
  /// 
...
class _ScrollablePositionedListState extends State<ScrollablePositionedList>
  ...
  ScrollController get scrollController => _showFrontList ? frontScrollController : backScrollController;
  ItemPositionsNotifier get itemPositionsNotifier => _showFrontList ? frontItemPositionsNotifier : backItemPositionsNotifier;
  ...
  Animatable<double> _opacityAnimation(_ListDisplay startListDisplay, List<double> opacityAnimationWeights) {
  ...
  void _updatePositions() {
    final itemPositions = itemPositionsNotifier.itemPositions.value
